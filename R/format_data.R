...
#'
#' @param site_ids `character` identifiers of sites.
#'
#' @param feature_ids `character` identifiers of features.
#'
#' @param action_ids `character` identifiers of actions.
#'
...
#'
#' @export
format_pu_data <- function(site_ids, feature_ids, action_ids,
  ...
  names(cost_data) <- paste0("cost_", action_ids)
  ...
  expectation_data <- lapply(seq_along(action_expectation_data), function(i) {
    ...
    names(out) <- paste0(action_ids[i], "_", feature_ids)
    out
  })
  ...
  do.call(
    ...
    append(
      list(
        tibble::tibble(site = site_ids), cost_data
      ),
      expectation_data
    )
  )
...
#'
#' @export
format_zone_data <- function(site_ids, feature_ids, action_ids,
                             parameters) {
  # assert arguments are valid
  assertthat::assert_that(
    is.character(site_ids),
  ...
  args <- lapply(action_ids, function(x) paste0(x, "_", feature_ids))

  # append feature and zone names to object
  args <- append(
    args,
    list(zone_names = action_ids, feature_names = feature_ids)
  )
...
#'
#' @export
format_target_data <- function(site_ids, feature_ids, action_ids,
  ...
  out <- tibble::tibble(
    feature = feature_ids,
    zone = list(action_ids)[rep(1, length(feature_ids))],
    type = "absolute",
    sense = ">=",
...
#'
#' @export
format_weights_data <- function(site_ids, feature_ids, action_ids,
                                feature_data, parameters) {
  # assert arguments are valid
  assertthat::assert_that(
    is.character(site_ids),
...
#'
#' @export
format_results_data <- function(site_ids, feature_ids, action_ids,
...
#'
#' @export
format_error_data <- function(site_ids, feature_ids, action_ids, problem,
                              parameters) {
  # assert arguments are valid
  assertthat::assert_that(
    is.character(site_ids),
  ...
  if (requireNamespace("gurobi", quietly = TRUE)) {
    ## prepare labels
    status_labels <- c(sapply(action_ids, function(x) {
      as.character(
        glue::glue(parameters$error_sheets$status_prefix,
          action_ids = x, site_ids = site_ids
        )
      )
    }))
    target_labels <-
